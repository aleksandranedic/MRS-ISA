...
export function HouseOwnerForm({show, setShow, owner, profileImg}) {
    const {id} = useParams();
    const form = useRef();
    const [showPassword, setShowPassword] = useState(false);
    const [validatedInfo, setValidatedInfo] = useState(false);
    const [houseOwner, setHouseOwner] = useState(owner);
    ...
    const setStreet = (value) => {
        setHouseOwner( prevState => {
            var newaddress = houseOwner.address
    ...
    const handleSubmit = e => {
        ...
        if (form.current.checkValidity() === false) {
            ...
            data.append("street", houseOwner.address.street)
            data.append("number", houseOwner.address.number)
        ...
        <Form ref={form} noValidate validated={validatedInfo}>
            ...
            <Modal.Body>
                ...
                <div className="d-flex mb-3">
                    ...
                    <div style={{width:"72%"}}>
                        <Form.Group className="mb-3 m-2" controlId="firstName">
                            ...
                            <Form.Control required type="text" name="firstName" defaultValue={houseOwner.firstName} onChange={e => setFirstName(e.target.value)}/>
                        ...
                        <Form.Group className="mb-3 m-2" controlId="lastName">
                            ...
                            <Form.Control required type="text" name="lastName" defaultValue={houseOwner.lastName} onChange={e => setLastName(e.target.value)}/>
                ...
                <Form.Group className="mb-3 m-2" controlId="phoneNumber">
                    ...
                    <Form.Control required type="text" name="phoneNumber" defaultValue={houseOwner.phoneNumber} onChange={e => setPhoneNumber(e.target.value)}/>
                ...
                <div className="d-flex">
                    <Form.Group className="mb-3 m-2">
                        ...
                        <InputGroup>
                        <Form.Control required className='w-50' type="text" defaultValue={houseOwner.address.street} onChange={e => setStreet(e.target.value)}/>
                        <Form.Control required className='w-50' type="text" defaultValue={houseOwner.address.number} onChange={e => setNumber(e.target.value)}/>
                    ...
                    <Form.Group className="mb-3 m-2" controlId="place">
                        ...
                        <Form.Control required type="text" defaultValue={houseOwner.address.place} onChange={e => setCity(e.target.value)}/>
                    ...
                    <Form.Group className="mb-3 m-2" controlId="country">
                        ...
                        <Form.Control required type="text" defaultValue={houseOwner.address.country} onChange={e => setCountry(e.target.value)}/>
...
export function ChangePassword({show, setShow}) {
    ...
    const changePassword = e => {
        ...
        if (formPassword.current.checkValidity() === false) {
            ...
            axios
            .post("http://localhost:4444/houseowner/checkPassword/" + id, oldPassword, {headers: {"Content-Type": "text/plain"}})
            .then(res => {
                if (res.data){
                    updatePassword();
                }
    ...
    const updatePassword = () => {
        ...
        axios
            .post("http://localhost:4444/houseowner/updatePassword/" + id, newPassword, {headers: {"Content-Type": "text/plain"}})
            .then(res => {
                window.location.reload();
            });
    }
...
export function DeleteAccount({show, setShow}) {
    ...
    const deleteUser = e => {
        ...
        if (formDelete.current.checkValidity() === false) {
            ...
            axios
            .delete(backLink + "/deletionRequests/houseowner/" + id, formData)
            .then(res => {
